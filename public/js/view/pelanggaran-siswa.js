(function($){
    ...
    $.ajax({
        ...
        success:function(pelanggaran){
            $('#pelanggaran_id').empty().append('<option value=""></option>');
            pelanggaran.forEach(v=>{
                $('#pelanggaran_id').append(`<option data-poin="${v.jenispelanggaran_poin}" value="${v._id}">(${v.jenispelanggaran_kode}) ${v.jenispelanggaran_nama}</option>`);
            });
        }
    })
        ...
        if(kelas_id!=''){
            $.ajax({
                ...
                success: function({siswa}){
                    ...
                    $('#siswa_id').parent().parent().fadeIn();
                }
            })
        }else{$('#siswa_id').val('').parent().parent().fadeOut();}
    })

    $('#pelanggaran_id').change(function(){
        let point = $(this).find(':selected').data('poin');
