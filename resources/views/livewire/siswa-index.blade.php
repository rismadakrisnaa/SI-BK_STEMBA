                        ...
                        <table class="table table-sm table-hover" data-form="dataForm">
                            ...
                            @foreach ($rows as $row)
                                <tr>
                                    ...
                                    <td>{{ $row->siswa_tmplahir }} / {{ \Carbon\Carbon::parse($row->siswa_tgllahir)->formatLocalized('%d %B %Y') }}</td>
                                    <td><img src="{{ asset('images') }}/{{ $row->siswa_aktif == 1 ? 1 : 0 }}.png" alt="[IMG]"></td>
                                    <td>
                                        <div class="d-flex float-right">
                                            <a href="{{ url('/dashboard/siswa/' . $row->_id) }}"
                                                class="btn btn-sm btn-info">
                                                <i class="fa fa-info-circle"></i>
                                                <span class="d-none d-lg-inline">DETAIL</span>
                                            </a>
                                            <a href="{{ url('/dashboard/siswa/' . $row->_id . '/edit') }}"
